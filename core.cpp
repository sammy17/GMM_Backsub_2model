...
int backsub(uint8_t frame_in[IMG_SIZE], uint8_t frame_out[76800], bool init,
		data_t para[IMG_SIZE * MODELS * 3]) {
//#pragma HLS DATAFLOW
...
//	static data_t para[IMG_SIZE*MODELS*3];
//#pragma HLS ARRAY_PARTITION variable=para block factor=128 dim=1
	//uint8_t iterator = 0;
	static data_t parameters[76800*MODELS*3/PARTS];

	for (int x = 0; x < PARTS; x++) {
#pragma HLS protocol fixed
		memcpy(parameters, &para[x * (IMG_SIZE * MODELS * 3 / PARTS)],
				sizeof(data_t)*IMG_SIZE * MODELS * 3 / PARTS);
...
#pragma HLS PIPELINE
	...
	memcpy(&para[x*((IMG_SIZE/PARTS)*MODELS*3)], parameters, sizeof(data_t)*(IMG_SIZE/PARTS)*MODELS*3);

	}
	return 0;
}

uint8_t EM_ALGO(uint8_t pixel, int pos,
		data_t parameters[(IMG_SIZE / PARTS) * MODELS * 3], int x) {
...
//#pragma HLS INLINE
	...
	for (int j = 0; j < MODELS; j++) {
		if ((abs(pixel - parameters[pos * MODELS * 3 + j])
				< 2.5 * hls::sqrtf(parameters[pos * MODELS * 3 + 2 + j]))
				and (back_gauss[x * (IMG_SIZE / PARTS) + pos][j])) {
			M[j] = true;
		}
		akt[j] = alpha_w / parameters[pos * MODELS * 3 + 4 + j];
		F[j] = parameters[pos * MODELS * 3 + 4 + j]
				/ parameters[pos * MODELS * 3 + 2 + j];
	}
	...
	if (max_val < MODELS) {	//For the largest F with M = 1
		//updating mean
		parameters[pos * MODELS * 3 + max_val] = parameters[pos * MODELS * 3
				+ max_val]
				+ akt[max_val]
						* (pixel - parameters[pos * MODELS * 3 + max_val]);
		...
		parameters[pos * MODELS * 3 + 2 + max_val] =
				parameters[pos * MODELS * 3 + 2 + max_val]
						+ akt[max_val]
								* ((pixel
										- parameters[pos * MODELS * 3 + max_val])
										* (pixel
												- parameters[pos * MODELS * 3
														+ max_val])
										- parameters[pos * MODELS * 3 + 2
												+ max_val]);

		//updating weight
		parameters[pos * MODELS * 3 + 4 + max_val] = parameters[pos * MODELS * 3
				+ 4 + max_val]
				- alpha_w * parameters[pos * MODELS * 3 + 4 + max_val]
				+ alpha_w;
		//std::cout << 	"********"<<	weight[pos][max_val] << " " << pos<<std::endl;
		matchsum[x * (IMG_SIZE / PARTS) + pos][max_val] = matchsum[x
				* (IMG_SIZE / PARTS) + pos][max_val] + 1;
...
#pragma HLS UNROLL
			...
			if (j != max_val) {
				//updating weight
				parameters[ MODELS * 3 * pos + 4 + j] = parameters[ MODELS * 3
						* pos + 4 + j] - alpha_w;
			}
		}
...
#pragma HLS UNROLL
			...
			if (j != min_val) {
				parameters[ MODELS * 3 * pos + 4 + j] = parameters[ MODELS * 3
						* pos + 4 + j] - alpha_w;
				matchsumtot = matchsumtot
						+ matchsum[x * (IMG_SIZE / PARTS) + pos][j]; // matchsumtot is the sum of the values of the matchsum of the K-1 Gaussians with highest F
			}
		}
		if (matchsumtot != 0) {
