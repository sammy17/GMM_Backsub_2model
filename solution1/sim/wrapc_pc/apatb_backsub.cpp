...
extern unsigned char matchsum[76800][2];

extern bool back_gauss[76800][2];
...
// wrapc file define: "para"
#define AUTOTB_TVIN_para  "../tv/cdatafile/c.backsub.autotvin_para.dat"
...
float para[460800])
{
	...
	if (wrapc_switch_file_token.good())
	{
		...
		if (atoi(AESL_num.c_str()) == AESL_transaction_pc)
		{
			...
			sc_bv<32> *gmem_offset_pc_buffer = new sc_bv<32>[460800];
			int i = 0;
			...
			if (i > 0)
			{
				// RTL Name: gmem_offset
				{
					...
					// {
						// celement: para(31, 0)
						// {
							sc_lv<32>* para_lv0_0_460799_1 = new sc_lv<32>[460800];
						// }
					// }
					...
					// bitslice(31, 0)
					{
						...
						{
							// carray: (0) => (460799) @ (1)
							for (int i_0 = 0; i_0 <= 460799; i_0 += 1)
							{
								if (&(para[0]) != NULL) // check the null address if the c port is array or others
								{
									para_lv0_0_460799_1[hls_map_index++].range(31, 0) = sc_bv<32>(gmem_offset_pc_buffer[hls_map_index].range(31, 0));
								}
							}
						}
					}
					...
					// bitslice(31, 0)
					{
						...
						// celement: para(31, 0)
						{
							...
							for (int i_0 = 0; i_0 <= 460799; i_0 += 1)
							{
								...
								// ori_name               : para[i_0]
								// sub_1st_elem           : 0
								// ori_name_1st_elem      : para[0]
								// output_left_conversion : *(int*)&para[i_0]
								// output_type_conversion : (para_lv0_0_460799_1[hls_map_index++]).to_uint64()
								if (&(para[0]) != NULL) // check the null address if the c port is array or others
								{
									*(int*)&para[i_0] = (para_lv0_0_460799_1[hls_map_index++]).to_uint64();
								}
							}
						}
					}
	...
	else
	{
		...
		// "para"
		char* tvin_para = new char[50];
		aesl_fh.touch(AUTOTB_TVIN_para);
		...
		sc_bv<32>* gmem_offset_tvin_wrapc_buffer = new sc_bv<32>[460800];
			...
			// bitslice(31, 0)
			{
				...
				// celement: para(31, 0)
				{
					...
					for (int i_0 = 0; i_0 <= 460799; i_0 += 1)
					{
						...
						// sub_1st_elem          : 0
						// ori_name_1st_elem     : para[0]
						// regulate_c_name       : para
						// input_type_conversion : *(int*)&para[i_0]
						if (&(para[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> para_tmp_mem;
							para_tmp_mem = *(int*)&para[i_0];
							gmem_offset_tvin_wrapc_buffer[hls_map_index++].range(31, 0) = para_tmp_mem.range(31, 0);
						}
					}
				}
			}
		...
		tcl_file.set_num(460800, &tcl_file.gmem_offset_depth);
		...
		// [[transaction]]
		sprintf(tvin_para, "[[transaction]] %d\n", AESL_transaction);
		aesl_fh.write(AUTOTB_TVIN_para, tvin_para);

		sc_bv<32> para_tvin_wrapc_buffer;
		...
		for (int i = 0; i < 1; i++)
		{
			sprintf(tvin_para, "%s\n", (para_tvin_wrapc_buffer).to_string(SC_HEX).c_str());
			aesl_fh.write(AUTOTB_TVIN_para, tvin_para);
		}

		tcl_file.set_num(1, &tcl_file.para_depth);
		sprintf(tvin_para, "[[/transaction]] \n");
		aesl_fh.write(AUTOTB_TVIN_para, tvin_para);

// [call_c_dut] ---------->

		int AESL_return = AESL_ORIG_DUT_backsub(frame_in, frame_out, init, para);
		...
		sc_bv<32>* gmem_offset_tvout_wrapc_buffer = new sc_bv<32>[460800];
				...
				// celement: para(31, 0)
				{
					...
					for (int i_0 = 0; i_0 <= 460799; i_0 += 1)
					{
						...
						if (&(para[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> para_tmp_mem;
							para_tmp_mem = *(int*)&para[i_0];
							gmem_offset_tvout_wrapc_buffer[hls_map_index++].range(31, 0) = para_tmp_mem.range(31, 0);
						}
					}
				}
			}
		...
		delete [] tvin_init;
		// release memory allocation: "para"
		delete [] tvin_para;
